...
const About = () => {
  const size = 200
  const [profileData, setProfileData] = React.useState<Partial<Profile> | null>(
    null
  )
  ...
  return (
    <div className="h-screen bg-dark flex flex-col md:flex-row items-center justify-center">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 md:gap-4">
        <SubSectionCenter>
          <h1
            className={`mb-[-50px] z-10 lg:mb-0 ml-0 lg:ml-8 text-[5rem] lg:text-[8rem] font-medium ${theSecrets.className}`}
          >
            Hello!
          </h1>
        </SubSectionCenter>
        ...
        <SubSectionCenter>
          <Image
            className="mx-0 md:mx-5 mb-0 md:mb-4 rounded-full"
            src={profile.src}
            width={size}
            height={size}
            alt="Profile Picture"
        ...
        <div className="w-[300px] ml-10 md:ml-0">
          <h2 className="my-2 text-[1.5rem] md:text-[2rem] font-medium">
            I'm
            <Show when={loading}>
              <Show.Then>
          ...
          <Show when={loading}>
            <Show.Then>
              <div className="h-32 md:h-52 flex flex-col justify-center">
